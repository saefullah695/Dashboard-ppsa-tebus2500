...
def load_and_process_data(spreadsheet_url: str, sheet_name: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # --- PERBAIKAN: Set bobot indikator sesuai yang ditentukan ---
        # Bobot per indikator
        indicator_weights = {
            'PSM': 20,
            'PWP': 25,
        ...
        # --- PERBAIKAN: Hitung ulang total score PPSA ---
...
def calculate_global_ppsa_score(df: pd.DataFrame) -> Dict:
    """
    --- PERBAIKAN UTAMA ---
    Menghitung Global PPSA Score dari semua data secara keseluruhan.
    Rumus: Global Score = Σ(Total Score PPSA per baris)
    Detail Kontribusi = (Total ACV × Bobot) / 100
    """
    if df.empty:
        return {
            'global_score': 0.0,
            'details': {
                'PSM': {'total_acv': 0.0, 'bobot': 20.0, 'kontribusi': 0.0},
                'PWP': {'total_acv': 0.0, 'bobot': 25.0, 'kontribusi': 0.0},
                'SG': {'total_acv': 0.0, 'bobot': 30.0, 'kontribusi': 0.0},
                'APC': {'total_acv': 0.0, 'bobot': 25.0, 'kontribusi': 0.0}
            }
        }
    
    # --- PERBAIKAN: Hitung Global Score dengan menjumlahkan semua score per baris ---
    total_score = 0.0
    if 'TOTAL SCORE PPSA (CORRECTED)' in df.columns:
        total_score = df['TOTAL SCORE PPSA (CORRECTED)'].sum()

    # --- PERBAIKAN: Gunakan bobot yang telah ditentukan ---
    indicator_weights = {
        'PSM': 20,
        'PWP': 25,
        'SG': 30,
    ...
    # Hitung TOTAL ACV untuk setiap indikator
    ...
    for indicator in indicators:
        ...
        if acv_col in df.columns:
            # --- PERBAIKAN: Hitung TOTAL ACV dari semua data ---
            total_acv = df[acv_col].sum()
            bobot = indicator_weights[indicator]
            
            # Kontribusi = (Total ACV × Bobot) / 100
            kontribusi = (total_acv * bobot) / 100
...
def main():
    ...
    with st.expander("🌍 Detail Global PPSA Score", expanded=False):
        ...
        for indicator, data in global_ppsa['details'].items():
            details_data.append({
                'Indikator': indicator,
                'Total ACV': format_percentage(data['total_acv']), # PERBAIKAN: Menampilkan Total ACV
        ...
        **Cara Perhitungan:**
        - Score per Indikator = (Total ACV × Bobot) / 100
